...
核心代码全部使用 Python，CLI + LLM 调用只需 `requests`，`pythonrt` 沙箱本体基于标准库；Web、语义搜索和沙箱内 Git 均为可选能力。较小的依赖面让运行逻辑更容易阅读、调试和替换，也让 XKAgent 能够参与修改自身。
...
无论采用哪种方式，启动时都应显式指定 `--workdir`。不传入 `--mode web` 时会进入 CLI；传入后则启动 Web。Web 默认监听 `127.0.0.1:7860`，默认用户名为 `admin`，仅在设置 `--password` 后启用认证。

### ⑤ 设置 Model

启动进入 CLI（或 Web）后，用 `/model` 命令查看或切换模型：
