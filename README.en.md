...
The core is written entirely in Python. The REPL and LLM integration require only `requests`, while the `pythonrt` sandbox itself uses the standard library. Web access, semantic search, and Git inside the sandbox are optional. A small dependency surface makes the runtime easier to read, debug, and replace—and allows XKAgent to help modify its own code.
...
For REPL-only use, install the minimal dependency:
...
Whichever method you use, always specify `--workdir` explicitly. Without `--mode web`, XKAgent starts the REPL; with it, XKAgent starts the Web interface. Web listens on `127.0.0.1:7860` by default, uses `admin` as the default username, and enables authentication only when `--password` is set.

### ⑤ Set the Model

After startup, use the `/model` command in the REPL (or Web) to view or switch models:
...
| [🏗️ Architecture Overview](.xkagent/docs/architecture-overview.md) |  | [💻 REPL and Web](.xkagent/docs/10-frontends.md) |
