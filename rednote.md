# XKAgent 图文介绍 📕

**XKAgent · eXtensible Kernel Agent** —— 纯 Python 的本地 Agent Harness

> Code over Tools · Skills 可插拔 · Status Info · 核心依赖只有 requests · CLI 与 Web 同一内核 · MIT 许可

> GitHub: <https://github.com/xtudbxk/XKAgent>

## 🔗 小红书原文

[📕 点击查看小红书原帖](https://www.xiaohongshu.com/explore/6a7ec001000000002c00599d?xsec_token=YBnWhA8rCHggjdoAAOzKJbv9Ue5q05GZo30h0weLSZ4io%3D&xsec_source=pc_creatormng)

---

## 📸 图文速览

### 封面

![preview 封面](.xkagent/docs/rednote/preview.png)


### 1. 项目简介与架构

![01 项目简介与架构](.xkagent/docs/rednote/01.png)


### 2. 纯 Python，一条依赖起步

![02 纯 Python，一条依赖起步](.xkagent/docs/rednote/02.png)


### 3. Web 端：执行过程完全可见

![03 Web 端：执行过程完全可见](.xkagent/docs/rednote/03.png)


### 4. CLI 端：全程键盘操作

![04 CLI 端：全程键盘操作](.xkagent/docs/rednote/04.png)


### 5. Feature 1 · Code over Tools

![05 Feature 1 · Code over Tools](.xkagent/docs/rednote/05.png)


### 6. Feature 2 · Skills 可插拔

![06 Feature 2 · Skills 可插拔](.xkagent/docs/rednote/06.png)


### 7. Feature 3 · Status Info

![07 Feature 3 · Status Info](.xkagent/docs/rednote/07.png)


### 8. Feature 4 · 多 Agent 协作

![08 Feature 4 · 多 Agent 协作](.xkagent/docs/rednote/11.png)


### 9. Feature 5 · 长程任务

![09 Feature 5 · 长程任务](.xkagent/docs/rednote/12.png)


### 10. 路径沙箱 · 渐进信任

![10 路径沙箱 · 渐进信任](.xkagent/docs/rednote/08.png)


### 11. Build Your Own XKAgent

![11 Build Your Own XKAgent](.xkagent/docs/rednote/09.png)


### 12. 总结 · 欢迎 star & fork

![12 总结 · 欢迎 star & fork](.xkagent/docs/rednote/10.png)


---

## 🚀 快速跳转

- [📕 小红书原帖](https://www.xiaohongshu.com/explore/6a7ec001000000002c00599d?xsec_token=YBnWhA8rCHggjdoAAOzKJbv9Ue5q05GZo30h0weLSZ4io%3D&xsec_source=pc_creatormng)
- [返回 README](README.md)

*XKAgent 欢迎 star，也欢迎 fork 成你自己的版本 ⭐*